...
namespace TypeCache.Tests.Extensions
{
	public class RangeExtensions
	{
		...
		public void Values()
		{
			Assert.Equal(new[] { 5, 6, 7 }, (5..8).Values().ToArray());
			Assert.Equal(new[] { 7, 6, 5 }, (7..4).Values().ToArray());
			Assert.Equal(new[] { 0, 1, 2, 3 }, (0..4).Values().ToArray());
			Assert.Equal(new[] { 4, 3, 2, 1 }, (4..0).Values().ToArray());
		}
	}
}
