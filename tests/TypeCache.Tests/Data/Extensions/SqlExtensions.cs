...
WHERE [ID] IN (1, 2, 3);
");
		var data = new Person[] { new() { ID = 1 }, new() { ID = 2 }, new() { ID = 3 } };
...
WHERE [First Name] = N'Sarah' AND [Last_Name] = N'Marshal';
");
		var actual = objectSchema.CreateDeleteSQL("[First Name] = N'Sarah' AND [Last_Name] = N'Marshal'", new[] { "DELETED.[ID]", "DELETED.[First Name]", "DELETED.[Last_Name]" });
...
");
		var actual = objectSchema.CreateInsertSQL<Person>(new[] { "FirstName", "LastName" }, data, new[] { "INSERTED.ID", "INSERTED.[LastName]" });
...
");
		var actual = objectSchema.CreateInsertSQL(new[] { "[First Name]", "[Last_Name]", "Age", "Amount" },
			selectQuery,
			new[] { "INSERTED.[First Name] AS [First Name]", "INSERTED.[ID] AS [ID]" });
...
");
		var actual = objectSchema.CreateUpdateSQL<Person>(new[] { "FirstName", "LastName" }, data
			, new[] { "INSERTED.[FirstName] AS [FirstName]", "DELETED.LastName AS LastName", "INSERTED.[ID] AS [ID]" });
...
");
		var actual = objectSchema.CreateUpdateSQL(new[] { "[First Name] = N'Sarah'", "Last_Name = N'Marshal'", "Age = @Param1" }
			...
			, new[] { "INSERTED.[First Name] AS [First Name]", "DELETED.[Last_Name] AS [Last_Name]", "INSERTED.ID AS [ID]" });
