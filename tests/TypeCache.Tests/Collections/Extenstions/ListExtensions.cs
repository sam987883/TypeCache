...
namespace TypeCache.Tests.Collections.Extensions
{
	public class ListExtensions
	{
		...
		public async Task IfAsync()
		{
			...
			Assert.Equal(new[] { 1, 3, 5 }, await list.IfAsync(async i => await Task.FromResult(i % 2 == 1)).ToListAsync());
			Assert.Empty(await list.IfAsync(async i => await Task.FromResult(i > 6)).ToListAsync());
			await Assert.ThrowsAsync<ArgumentNullException>(async () => await list.IfAsync(null).ToListAsync());
