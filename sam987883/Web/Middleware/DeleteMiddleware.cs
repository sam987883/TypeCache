...
namespace sam987883.Web.Middleware
{
	public class DeleteMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext, ISchemaStore schemaStore, IRequestValidator<Delete> requestValidator)
        {
            var request = await JsonSerializer.DeserializeAsync<Delete>(httpContext.Request.Body);
