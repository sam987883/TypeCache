...
public static class QueryArgumentsExtensions
{
	public static void Add<T>(this QueryArguments @this, string name, bool? nullable = null, object? defaultValue = null, string? description = null)
	{
		...
		var isList = typeof(T) != typeof(string) && (typeof(T).IsArray || typeof(T).Implements(typeof(IEnumerable<>).MakeGenericType(typeof(T))));
		var type = typeof(T) switch
		{
			{ IsArray: true } when typeof(T) != typeof(string) => typeof(T).GetElementType()!,
		...
		type = type switch
		{
			{ IsEnum: true } => typeof(GraphQLEnumType<>).MakeGenericType(type),
			_ when type.Implements(typeof(ISpanParsable<>)) => typeof(GraphQLScalarType<>).MakeGenericType(type),
			_ when type == typeof(string) => typeof(GraphQLStringType),
			_ when type == typeof(bool) => typeof(GraphQLBooleanType),
			_ => typeof(GraphQLInputType<>).MakeGenericType(type)
		};
