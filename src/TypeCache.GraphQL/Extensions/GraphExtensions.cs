...
namespace TypeCache.GraphQL.Extensions
{
	public static class GraphExtensions
    {
        ...
        public static Type GetGraphType(this IMember @this, bool isInput)
        {
            ...
            {
                IMethodMember method => method.ReturnAttributes.First<Attribute, GraphTypeAttribute>()?.Type,
                IStaticMethodMember method => method.ReturnAttributes.First<Attribute, GraphTypeAttribute>()?.Type,
                _ => @this.Attributes.First<Attribute, GraphTypeAttribute>()?.Type
            };
            ...
            if ((!@this.IsNullable && @this.NativeType != NativeType.Object)
                || @this.Attributes.Any<Attribute, GraphRequireAttribute>())
        ...
        public static QueryArguments ToQueryArguments(this IMethodMember @this)
            => new QueryArguments(@this.Parameters
                    .If(parameter => !parameter.Attributes.Any<Attribute, GraphIgnoreAttribute>())
                    .If(parameter => !parameter.Attributes.Any<Attribute, GraphInjectAttribute>())
                    .To(parameter => parameter.ToQueryArgument()));

        private static QueryArgument ToQueryArgument(this IParameter @this)
        {
            var graphType = @this.Attributes.First<Attribute, GraphTypeAttribute>()?.Type;
            if (graphType == null)
            {
            ...
            {
                Name = @this.Name,
                Description = @this.Attributes.First<Attribute, GraphDescriptionAttribute>()?.Description
            };
        }
    }
}
