...
public static class SchemaExtensions
{
	...
	{
		var document = schema.ToAST();
		options ??= new();
		if (!options.IncludeDeprecationReasons)
			RemoveDeprecationReasonsVisitor.Visit(document);

		if (!options.IncludeDescriptions)
			RemoveDescriptionsVisitor.Visit(document);

		if (!options.IncludeFederationTypes)
			RemoveFederationTypesVisitor.Visit(document);

		if (options.StringComparison is not null)
			SDLSorter.Sort(document, new(options.StringComparison.Value));

		var printer = new SDLPrinter(options);
		return printer.PrintAsync(document, writer, cancellationToken);
	}
	...
	public static TSchema EnableExperimentalIntrospectionFeatures<TSchema>(this TSchema schema, ExperimentalIntrospectionFeaturesMode mode = ExperimentalIntrospectionFeaturesMode.ExecutionOnly)
		where TSchema : ISchema
	{
		schema.Initialized.ThrowIfTrue();
		...
		if (mode is ExperimentalIntrospectionFeaturesMode.IntrospectionAndExecution)
	...
	public static async Task<string> ExecuteAsync(this ISchema schema, IGraphQLTextSerializer serializer, Action<ExecutionOptions> configure)
	{
		...
		var executor = (IDocumentExecuter)new DocumentExecuter();
	...
	public static void Run(this ISchemaNodeVisitor visitor, ISchema schema)
	{
		...
		foreach (var directive in schema.Directives)
		{
			...
			directive.Arguments.ForEach(argument => visitor.VisitDirectiveArgumentDefinition(argument, directive, schema));
		}
		...
		foreach (var item in schema.AllTypes.Dictionary)
		{
			switch (item.Value)
			{
				case GraphQLEnumType e:
					visitor.VisitEnum(e, schema);
					foreach (var pair in e.Values) //ISSUE:allocation
						visitor.VisitEnumValue(pair.Value, e, schema);
					break;
				...
				case IInterfaceGraphType iface:
					...
					foreach (var field in iface.Fields)
					{
						...
						field.Arguments.ForEach(argument => visitor.VisitInterfaceFieldArgumentDefinition(argument, field, iface, schema));
					}
					break;
				...
				case IObjectGraphType output:
					...
					foreach (var field in output.Fields)
					{
						...
						field.Arguments.ForEach(argument => visitor.VisitObjectFieldArgumentDefinition(argument, field, output, schema));
					}
					break;
	...
	private sealed class ReplaceScalarVisitor : BaseSchemaNodeVisitor
	{
		...
		private void Replace(IProvideResolvedType provider)
		{
			if (provider.ResolvedType is IProvideResolvedType wrappedProvider)
				this.Replace(wrappedProvider);
			else if (provider.ResolvedType is ScalarGraphType scalar && scalar.Name.EqualsIgnoreCase(this._replacement.Name))
				provider.ResolvedType = this._replacement;
		}
	}
