...
/// </summary>
public class ReadonlyResolveFieldContext : IResolveFieldContext
{
	...
	private IDictionary<string, ArgumentValue>? GetArguments()
		=> ExecutionHelper.GetArguments(_executionNode.FieldDefinition!.Arguments, _executionNode.Field!.Arguments!.ToArray(), _executionContext.Variables);
	...
	public IResolveFieldContext? Parent
	{
		get
		{
			if (_parent is null)
			{
				...
				if (parent is not null && parent is not RootExecutionNode)
	...
	/// <inheritdoc/>
	public IList<ExecutionError> Errors => _executionContext.Errors;
	...
	/// <inheritdoc/>
	public IDictionary<string, object> InputExtensions => _executionContext.InputExtensions;
