...
/// <summary>
/// No fragment cycles: A GraphQL document is only valid if it does not contain fragment cycles.
/// </summary>
		...
		ValidationContext context)
	{
		...
		foreach (var spreadNode in spreadNodes)
		{
			...
			else
			{
				...
				var nodes = cyclePath.OfType<ASTNode>().Append(spreadNode).ToArray();

				context.ReportError(new NoFragmentCyclesError(context, spreadName.StringValue, cyclePath.Select(_ => _.FragmentName.Name.StringValue).ToArray(), nodes)); // ISSUE:allocation
			}
		}
