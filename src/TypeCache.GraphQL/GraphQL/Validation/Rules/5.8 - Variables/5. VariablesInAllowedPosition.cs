...
public class VariablesInAllowedPosition : IValidationRule
{
			...
			leave: (op, context) =>
			{
				...
				if (usages is not null)
				{
					foreach (var usage in usages)
					{
						...
						if (!varDefMap.TryGetValue(varName, out var varDef) || varDef is null)
							return;

						var varType = varDef.Type.GraphTypeFromType(context.Schema);

						if (varType is not null && usage.Type is not null && !IsVariableUsageAllowed(varDef, varType, usage, context.Schema.Features.AllowScalarVariablesForListTypes))
	...
	private static bool IsVariableUsageAllowed(GraphQLVariableDefinition variableDefinition, IGraphType variableType, VariableUsage variableUsage, bool allowScalarsForLists)
	{
		...
		if (variableUsage.Type is NonNullGraphType nonNullUsageType && variableType is not NonNullGraphType)
		{
			...
			var hasNonNullVariableDefaultValue = variableDefinition.DefaultValue is not null;
			...
			return variableType.IsSubtypeOf(nullableLocationType);
		}

		// >> Return AreTypesCompatible(variableType, locationType).
		return variableType.IsSubtypeOf(variableUsage.Type);
	}
}
