...
internal sealed class CompositeVariableVisitor : IVariableVisitor
{
	...
	public async ValueTask VisitFieldAsync(ValidationContext context, GraphQLVariableDefinition variable, VariableName variableName, IInputObjectGraphType type, FieldType field, object? variableValue, object? parsedValue)
	{
		foreach (var visitor in _visitors)
			await visitor.VisitFieldAsync(context, variable, variableName, type, field, variableValue, parsedValue);
	}
	...
	{
		foreach (var visitor in _visitors)
			await visitor.VisitListAsync(context, variable, variableName, type, variableValue, parsedValue);
	}
	...
	{
		foreach (var visitor in _visitors)
			await visitor.VisitObjectAsync(context, variable, variableName, type, variableValue, parsedValue);
	}
	...
	{
		foreach (var visitor in _visitors)
			await visitor.VisitScalarAsync(context, variable, variableName, type, variableValue, parsedValue);
	}
}
