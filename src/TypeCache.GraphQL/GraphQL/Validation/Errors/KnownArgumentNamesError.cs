...
public class KnownArgumentNamesError : ValidationError
{
	...
	/// </summary>
	public KnownArgumentNamesError(ValidationContext context, GraphQLArgument node, IFieldType fieldDef, IGraphType parentType)
		: base(context.Document.Source, NUMBER,
			UnknownArgMessage(
				node.Name.StringValue,
				fieldDef.Name,
				parentType.ToString()!,
				StringUtils.SuggestionList(node.Name.StringValue, fieldDef.Arguments.Select(_ => _.Name))), //ISSUE:allocation
			node)
	{
	}
	...
	public KnownArgumentNamesError(ValidationContext context, GraphQLArgument node, Directive directive)
		: base(context.Document.Source, NUMBER,
			UnknownDirectiveArgMessage(
				node.Name.StringValue,
				directive.Name,
				StringUtils.SuggestionList(node.Name.StringValue, directive.Arguments?.Select(_ => _.Name))), // ISSUE:allocation
			node)
	{
	}
