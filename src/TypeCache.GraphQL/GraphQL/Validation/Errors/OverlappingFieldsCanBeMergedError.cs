...
public class OverlappingFieldsCanBeMergedError : ValidationError
{
	...
	internal static string FieldsConflictMessage(string responseName, ConflictReason reason)
		=> $"Fields {responseName} conflicts because {ReasonMessage(reason.Message)}. Use different aliases on the fields to fetch both if this was intentional.";

	private static string ReasonMessage(Message reasonMessage)
		=> reasonMessage.Msgs?.Count > 0
			? string.Join(" and ", reasonMessage.Msgs.Select(x => $"subfields '{x.Name}' conflict because {ReasonMessage(x.Message)}"))
			: reasonMessage.Msg!;
}
