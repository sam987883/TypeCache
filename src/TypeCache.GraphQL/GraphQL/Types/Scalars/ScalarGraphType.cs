...
public abstract class ScalarGraphType : GraphType
{
	...
	{
		GraphQLBooleanValue b => ParseValue(b.BoolValue.Box()),
		GraphQLIntValue i => long.Parse(i.Value),
	...
	public virtual bool IsValidDefault(object value)
	{
		...
		try
		{
			return ToAST(value) is not null;
		}
		catch
		{
			return false;
	...
	public virtual GraphQLValue ToAST(object? value)
		=> this.Serialize(value) switch
		{
			...
			Uri x => new GraphQLStringValue(x.ToString()),
			_ => throw new NotImplementedException($"Please override the '{nameof(ToAST)}' method of the '{this.GetType().Name}' scalar to support this operation.")
		};
	...
	protected internal GraphQLValue ThrowASTConversionError(object? value)
		=> throw new InvalidOperationException($"Unable to convert '{value ?? "null"}' to its AST representation for the scalar type '{this.Name}'.");
	...
	protected object ThrowLiteralConversionError(GraphQLValue input, string? description = null)
	{
		...
		throw new InvalidOperationException($"Unable to convert '{value}' literal from AST representation to the scalar type '{this.Name}'; {description}");
	}
	...
	protected object ThrowValueConversionError(object? value)
		=> throw new InvalidOperationException($"Unable to convert '{value ?? "null"}' value to the scalar type '{Name}'");
	...
	protected object ThrowSerializationError(object? value)
		=> throw new InvalidOperationException($"Unable to serialize '{value ?? "null"}' value to the scalar type '{Name}'.");
