namespace GraphQL.Types
{
	...
	public interface IAbstractGraphType : IGraphType
	{
		...
		/// </summary>
		ISet<IObjectGraphType> PossibleTypes { get; }
	...
	public static class AbstractGraphTypeExtensions
	{
		...
		{
			foreach (var possibleType in abstractType.PossibleTypes)
			{
		...
		{
			var result = abstractType.ResolveType is not null
			...
			static IObjectGraphType? getTypeOf(IAbstractGraphType abstractType, object value)
			{
				foreach (var possibleType in abstractType.PossibleTypes)
				{
					if (possibleType.TypeHandle.Equals(value.GetType().TypeHandle))
						return possibleType;
				}
