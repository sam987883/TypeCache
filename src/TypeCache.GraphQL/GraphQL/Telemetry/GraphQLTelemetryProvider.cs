...
public class GraphQLTelemetryProvider : IConfigureExecution
{
    ...
    /// </summary>
    public const string SourceName = "GraphQL";
    ...
    /// <inheritdoc/>
    public virtual float SortOrder { get; } = 200F;

    /// <inheritdoc/>
    public virtual Task<ExecutionResult> ExecuteAsync(ExecutionOptions options, ExecutionDelegate next)
		=> !_onlyWhenAutoTelemetryEnabled || OpenTelemetry.AutoInstrumentation.Initializer.Enabled ? ExecuteInternalAsync(options, next) : next(options);
    ...
    private async Task<ExecutionResult> ExecuteInternalAsync(ExecutionOptions options, ExecutionDelegate next)
    {
        ...
        using var activity = await StartActivityAsync(options);
        ...
        if (!await FilterAsync(options, activity))
            return await next(options);

        // record the requested operation name and optionally the GraphQL document
        await SetInitialTagsAsync(activity, options);
        ...
        catch (Exception ex)
        {
            ...
            if (ex is not OperationCanceledException)
                await OnExceptionAsync(activity, ex);
            throw;
        }

        // record the status
        await SetResultTagsAsync(activity, options, result);
    ...
    {
        if (options.OperationName is not null)
    ...
    protected virtual Task SetOperationTagsAsync(Activity activity, ExecutionOptions options, ISchema schema, GraphQLDocument document, GraphQLOperationDefinition operation)
    {
        ...
        activity.DisplayName = operationName is null ? operationType : $"{operationType} {operationName}";
    ...
    // TODO: note: this could be implemented as a validation rule with no public API changes
