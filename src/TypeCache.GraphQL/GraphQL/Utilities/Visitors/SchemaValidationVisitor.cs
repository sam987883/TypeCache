...
public sealed class SchemaValidationVisitor : BaseSchemaNodeVisitor
{
	...
	public override void VisitObject(IObjectGraphType type, ISchema schema)
	{
		...
		// 2.1
		foreach (var item in type.Fields.ToLookup(_ => _.Name))
		{
			if (item.Count() > 1)
	...
	public override void VisitObjectFieldDefinition(FieldType field, IObjectGraphType type, ISchema schema)
	{
		...
		if (field.StreamResolver is not null && type != schema.Subscription)
	...
	public override void VisitInputObjectFieldDefinition(FieldType field, IInputObjectGraphType type, ISchema schema)
	{
		...
		else if (field.DefaultValue is not null && !field.ResolvedType!.IsValidDefault(field.DefaultValue))
		{
		...
		if (field.StreamResolver is not null)
	...
	public override void VisitSchema(ISchema schema)
	{
		...
		if (n1 == n2 && n1 is not null || n1 == n3 && n1 is not null || n2 == n3 && n2 is not null)
		...
		if (schema.Subscription is not null)
		{
			...
			{
				if (field.StreamResolver is null)
	...
	/// <inheritdoc/>
	public override void VisitEnum(GraphQLEnumType type, ISchema schema)
	{
		// 1
	...
	public override void VisitDirectiveArgumentDefinition(QueryArgument argument, Directive directive, ISchema schema)
	{
		...
		else if (argument.DefaultValue is not null && !argument.ResolvedType!.IsValidDefault(argument.DefaultValue))
	...
	private void ValidateFieldArgumentsUniqueness(FieldType field, INamedType type)
	{
		if (field.Arguments.Length > 0)
		{
			foreach (var item in field.Arguments.ToLookup(_ => _.Name))
			{
				if (item.Count() > 1)
	...
	private void ValidateDirectiveArgumentsUniqueness(Directive directive)
	{
		...
		{
			foreach (var item in directive.Arguments.ToLookup(_ => _.Name))
			{
				if (item.Count() > 1)
