...
public class __InputValue : ObjectGraphType<IProvideMetadata> // context.Source either QueryArgument or FieldType
{
	...
	{
		this.SetName(nameof(__InputValue), validate: false);

		this.Description = "Arguments provided to Fields or Directives and the input fields of an InputObject are represented as Input Values which describe their type and optionally a default value.";
		...
		this.Field<NonNullGraphType<__Type>>("type").Resolve(context => ((IProvideResolvedType)context.Source!).ResolvedType);

		this.Field<GraphQLStringType>("defaultValue")
			.Description("A GraphQL-formatted string representing the default value for this input value.")
			.Resolve(context =>
			{
				return context.Source is IHaveDefaultValue hasDefault && hasDefault.DefaultValue is not null
		...
		if (deprecationOfInputValues)
		{
			...
			this.Field<NonNullGraphType<GraphQLBooleanType>>("isDeprecated").Resolve(context => ((IProvideDeprecationReason)context.Source).DeprecationReason.IsNotBlank().Box());
			this.Field<GraphQLStringType>("deprecationReason").Resolve(context => ((IProvideDeprecationReason)context.Source).DeprecationReason);
		}
