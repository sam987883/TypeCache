...
public class SubscriptionExecutionStrategy : ExecutionStrategy
{
	...
	{
		baseExecutionStrategy.ThrowIfNull();

		this._baseExecutionStrategy = baseExecutionStrategy;
	}
	...
	private async Task<IDictionary<string, IObservable<ExecutionResult>>?> ExecuteSubscriptionNodesAsync(ExecutionContext context, ExecutionNode[] nodes)
	{
		...
		{
			var stream = await ResolveResponseStreamAsync(context, node);
			if (stream is null)
				return null;
	...
	protected virtual async Task<IObservable<ExecutionResult>?> ResolveResponseStreamAsync(ExecutionContext context, ExecutionNode node)
	{
		...
		try
		{
			...
			if (resolver is null) // TODO: this should be caught by schema validation
				throw new InvalidOperationException($"Stream resolver not set for field '{node.Field.Name}'.");

			sourceStream = await resolver.ResolveAsync(resolveContext);
		...
		catch (Exception exception)
		{
			context.Errors.Add(await HandleExceptionInternalAsync(context, node, exception,
				$"Could not resolve source stream for field '{node.Field.Name}'."));
			return null;
		}
		...
		return sourceStream
			...
			.SelectCatchAsync(
				async (value, token) =>
				{
					...
					var childContext = CloneExecutionContext(preservedContext, token);

					return await ProcessDataAsync(childContext, node, value);
				},
				async (exception, token) =>
				{
					var childContext = CloneExecutionContext(preservedContext, token);

					return await ProcessErrorAsync(childContext, node, exception);
				});
	}
	...
	protected virtual async ValueTask<ExecutionResult> ProcessDataAsync(ExecutionContext context, ExecutionNode node, object? value)
	{
		...
		try
		{
			...
			await ExecuteNodeTreeAsync(context, node);
		...
		catch (OperationCanceledException) when (context.CancellationToken.IsCancellationRequested)
		{
			...
			result.Errors.Add(GenerateError(context, node, "The operation was canceled."));
		}
		...
		{
			result.Errors.Add(await HandleExceptionInternalAsync(context, node, exception,
				$"Could not process source stream event data for field '{node.Field.Name}'."));
		}
	...
	{
		var executionError = await HandleExceptionAsync(context, node, exception, defaultMessage);
		executionError.Path = node.ResponsePath;
		executionError.AddLocation(node.Field, context.Document);
	...
	protected virtual async Task<ExecutionError> HandleExceptionAsync(ExecutionContext context, ExecutionNode node, Exception exception, string defaultMessage)
	{
		...
		return exception is ExecutionError executionError ? executionError : new UnhandledError(exceptionContext?.ErrorMessage ?? defaultMessage, exception);
	}
