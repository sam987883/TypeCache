...
public abstract class ExecutionStrategy : IExecutionStrategy
{
	...
	protected virtual bool ShouldIncludeNode(ExecutionContext context, IHasDirectivesNode node)
	{
		...
		{
			var directive = node.Directives.Find(context.Schema.Directives.Skip.Name);
			if (directive is not null)
			{
				var arg = context.Schema.Directives.Skip["if"]!;
				if (ExecutionHelper.CoerceValue(arg.ResolvedType!, directive.Arguments?.ValueFor(arg.Name), context.Variables, arg.DefaultValue).Value is true)
					return false;
			}

			directive = node.Directives.Find(context.Schema.Directives.Include.Name);
			if (directive is not null)
			{
				var arg = context.Schema.Directives.Include["if"]!;
				return ExecutionHelper.CoerceValue(arg.ResolvedType!, directive.Arguments?.ValueFor(arg.Name), context.Variables, arg.DefaultValue).Value is true;
			}
		}
	...
	{
		Debug.Assert(parent.Result is not null || parent is RootExecutionNode);
		...
		{
			(var field, var fieldDefinition) = pair.Value;
	...
	public virtual Dictionary<string, (GraphQLField field, FieldType fieldType)>? GetSubFields(ExecutionContext context, ExecutionNode node)
		=> node.Field?.SelectionSet?.Selections?.Count > 0
			? this.CollectFieldsFrom(context, node.FieldDefinition!.ResolvedType!, node.Field.SelectionSet, null)
			: null;
	...
	protected virtual Dictionary<string, (GraphQLField field, FieldType fieldType)> CollectFieldsFrom(ExecutionContext context, IGraphType specificType, GraphQLSelectionSet selectionSet, Dictionary<string, (GraphQLField field, FieldType fieldType)>? fields)
	{
		...
		CollectFields(context, specificType.GetNamedGraphType(), selectionSet, fields, visitedFragmentNames, 0);

		if (visitedFragmentNames is not null)
				...
				foreach (var selection in selectionSet.Selections)
				{
					if (selection is GraphQLField field)
					{
						if (ShouldIncludeNode(context, field))
						{
							...
							var fieldType = GetFieldDefinition(context.Schema, (specificType as IObjectGraphType)!, field)
					...
					{
						if ((visitedFragmentNames is null || !Contains(visitedFragmentNames, spread.FragmentName.Name, foundFragments)) && ShouldIncludeNode(context, spread))
						{
							if (visitedFragmentNames is not null)
							...
							if (fragment is not null && ShouldIncludeNode(context, fragment) && DoesFragmentConditionMatch(context, fragment.TypeCondition.Type.Name, specificType))
					...
					else if (selection is GraphQLInlineFragment inline)
					{
						...
						if (ShouldIncludeNode(context, inline) && DoesFragmentConditionMatch(context, inline.TypeCondition is not null ? inline.TypeCondition.Type.Name : specificType.Name, specificType))
		...
		{
			string name = field.Alias is not null ? field.Alias.Name.StringValue : fieldType.Name; //ISSUE:allocation in case of alias
	...
	protected virtual async Task SetArrayItemNodesAsync(ExecutionContext context, ArrayExecutionNode parent)
	{
		...
		if (data is IList list)
		{
			for (int i = 0; i < list.Count; ++i)
				await SetArrayItemNode(list[i]);
		}
		else
		{
			foreach (object? d in data)
				await SetArrayItemNode(d);
		}
		...
		async Task SetArrayItemNode(object? d)
		{
			...
			await CompleteNodeAsync(context, node);
	...
	protected virtual async Task ExecuteNodeAsync(ExecutionContext context, ExecutionNode node)
	{
		...
		try
		{
			...
			resolveContext = resolveContext is not null ? resolveContext.Reset(node, context) : new ReadonlyResolveFieldContext(node, context);

			var result = await node.FieldDefinition.Resolver!.ResolveAsync(resolveContext);
			node.Result = result;
		...
		{
			if (await ProcessNodeUnhandledExceptionAsync(context, node, ex))
				throw;
		}
	}
	...
	protected virtual async Task CompleteNodeAsync(ExecutionContext context, ExecutionNode node)
	{
		try
		{
			...
			if (node.Result is not null)
			{
				...
				{
					await SetArrayItemNodesAsync(context, arrayNode);
				}
			}
		}
		catch (OperationCanceledException) when (context.CancellationToken.IsCancellationRequested)
	...
	protected virtual async Task<bool> ProcessNodeUnhandledExceptionAsync(ExecutionContext context, ExecutionNode node, Exception ex)
	{
		...
		if (context.UnhandledExceptionDelegate is not null)
		{
			...
			await context.UnhandledExceptionDelegate(exceptionContext);
	...
	{
		var fieldType = node.ResolvedType;
		if (fieldType is NonNullGraphType nonNullType)
		{
			if (node.Result is null)
				throw new InvalidOperationException("Cannot return null for a non-null type. Field: {node.Name}, Type: {nonNullType}.");
		...
		{
			objectType = abstractType.GetObjectType(node.Result, context.Schema);
			if (objectType is null)
			{
				var type = (node.IndexInParentNode.HasValue ? node.Parent?.Parent : node.Parent)?.GraphType?.Name;
				var field = node.FieldDefinition.Name;
				throw new InvalidOperationException(
					$"Abstract type {abstractType.Name} must resolve to an Object type at runtime for field {type}.{field} with value '{node.Result}'.");
			}
		...
		//if (objectType?.TypeHandle.ToType() != node.Result.GetType())
		if (objectType?.GetType().IsGenericType is true
			&& objectType?.GetType().GenericTypeArguments[0] != node.Result.GetType())
			throw new InvalidOperationException($"'{node.Result}' value of type '{node.Result.GetType()}' is not allowed for '{objectType?.Name}'.");
	}
}
