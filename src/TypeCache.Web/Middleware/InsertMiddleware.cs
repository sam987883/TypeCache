...
{
	public class InsertMiddleware
    {
        private readonly string _ConnectionString;
        private readonly DbProviderFactory _DbProviderFactory;

        public InsertMiddleware(RequestDelegate _, string providerName, string connectionString)
        {
        ...
        public async Task Invoke(HttpContext httpContext, IServiceProvider serviceProvider, ISchemaStore schemaStore)
        {
            ...
            var request = await JsonSerializer.DeserializeAsync<InsertRequest>(httpContext.Request.Body, jsonOptions);
            var requestValidator = serviceProvider.GetService<IRequestValidator<InsertRequest>>();
            ...
            if (valid)
            {
                ...
                var output = connection.Insert(request, schemaStore);
