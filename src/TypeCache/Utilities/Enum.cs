...
{
	[DebuggerHidden]
	public static IReadOnlySet<Attribute> Attributes { get; } = typeof(T).GetCustomAttributes(false).Cast<Attribute>().ToImmutableHashSet();
	...
	[DebuggerHidden]
	public static IReadOnlySet<T> Values { get; } = Enum.GetValues<T>().ToImmutableHashSet(new EnumComparer<T>());
