	...
	where T : class
{
	...
	public static long Add(T instance, string eventMemberName, Delegate handler)
	{
		...
		eventInfo.AddMethod.InvokeMethod(new object[] { instance, handler });
		return key;
	}
	...
	public static bool Remove(long key)
	{
		...
		if (handler.Instance.TryGetTarget(out var target))
			handler.RemoveMethodHandle?.ToMethodBase().InvokeMethod(new object[] { target, handler.EventHandler });
