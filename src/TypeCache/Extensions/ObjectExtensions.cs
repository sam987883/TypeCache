	...
	public static class ObjectExtensions
	{
		...
		public static TypeMember GetTypeMember<T>(this T @this)
			...
			=> (@this switch
			{
				...
				MemberInfo memberInfo => memberInfo.DeclaringType!.TypeHandle,
				_ => @this.GetType().TypeHandle
			}).GetTypeMember();
		...
		public static void MapFields<T>(this T @this, T source)
		{
			if (source is not null)
				@this?.GetType().GetTypeMember().Fields
					.If(_ => _.Value.Public && _.Value.Getter is not null && _.Value.Setter is not null)
		...
		public static IDictionary<string, object?>? ToFieldDictionary<T>(this T @this, StringComparison comparison = StringComparison.OrdinalIgnoreCase)
			=> @this?.GetType().GetTypeMember().Fields.If(_ => _.Value.Getter is not null).ToDictionary(_ => _.Key, _ => _.Value.GetValue(@this), comparison.ToStringComparer());
		...
		public static IDictionary<string, object?>? ToPropertyDictionary<T>(this T @this, StringComparison comparison = StringComparison.OrdinalIgnoreCase)
			=> @this?.GetType().GetTypeMember().Properties.If(_ => _.Value.Getter is not null).ToDictionary(_ => _.Key, _ => _.Value.GetValue(@this), comparison.ToStringComparer());
	}
}
