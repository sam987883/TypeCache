...
internal class PropertyMapper : IMapper
{
		...
		where T : class
	{
		...
		{
			var property = targetProperties[pair.Key];
			...
			{
				targetValue = property.PropertyType.Create();
				if (targetValue is null)
					continue;
		...
		where S : class
	{
		...
		foreach (var sourceProperty in Type<S>.Properties.Values.Where(_ => _.CanRead))
		{
			...
			var mapAttributes = sourceProperty.Attributes.OfType<MapAttribute>()
			...
			{
				target[sourceProperty.Name] = value;
			}
		}
	}
		...
		where T : class
	{
		...
		{
			var property = Type<T>.Properties[propertyName];
			if (property?.CanWrite is not true)
				return;
	...
	public void Map(object source, object target)
	{
		...
		void setPropertyValue(string propertyName, object? value)
		{
			...
			{
				var targetValue = property.GetValue(target) ?? property.PropertyType.Create();
